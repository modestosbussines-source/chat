...
After logging in, you receive an access token and refresh token. Include the access token in the `Authorization` header for all protected API requests. Alternatively, use an [API key](/omni/api-reference/api-keys) in the `X-API-Key` header.
